...
export default function BlocForm({
  ...
  const getInitialFormData = useCallback((): BlocFormData => {
    if (mode === "edit" && bloc) {
      return {
        name: bloc.name || "",
  ...
  const [formData, setFormData] = React.useState<BlocFormData>(
    getInitialFormData()
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error(
        `Error ${mode === "create" ? "creating" : "updating"} bloc:`,
        error
      );
      alert(
        `Erreur lors de la ${
          mode === "create" ? "création" : "mise à jour"
        } du bloc. Veuillez réessayer.`
      );
    }
  };
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  handleInputChange("lotissement_id", value)
                }
                required
              >
              ...
              {(!lotissements || lotissements.length === 0) && (
                <p className="text-xs text-orange-500">
                  Aucun lotissement disponible. Créez d&apos;abord un
                  lotissement.
                </p>
              )}
            </div>
