...
export const documentService = {
    ...
    async getByParcelleId(parcelle_id:number):Promise<Document>{
        const response = await apiClient.get<Document>(PARCELLEDOC_URL + `/parcelleDoc?parcelle_id=${parcelle_id}`)
        return response;
    },

    async getById(doc_id:number){
        const response = await apiClient.get(PARCELLEDOC_URL+"/"+String(doc_id)+"/")
        return response;
    },
    ...
    async put(doc_id:number, document:any){
        const response = apiClient.post(PARCELLEDOC_URL+"/"+String(doc_id)+"/", document);
        return response;
    },

    async get():Promise<Document[]>{
        const response = await apiClient.get<APIDocumentResponse>(PARCELLEDOC_URL+"/")
